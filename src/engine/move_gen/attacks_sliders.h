...
constexpr inline Bitboard rook_attacks_ray(Square sq, Bitboard occupancy) noexcept {
    ...
    for (int r = curRank + 1; r <= 7; ++r) {
        const Square to = make_square(static_cast<File>(curFile), static_cast<Rank>(r));
        attacks |= bitboard(to);
    ...
    for (int f = curFile + 1; f <= 7; ++f) {
        const Square to = make_square(static_cast<File>(f), static_cast<Rank>(curRank));
        attacks |= bitboard(to);
        if (occupancy & bitboard(to))
            break;
    }
...
constexpr inline void init_rook_attacks_table() noexcept {
    for (size_t i = 0; i < 64; ++i) {
        ...
        const size_t relevantBlockerCount = 1ULL << (64 - magic.shift);

        for (size_t j = 0; j < relevantBlockerCount; ++j) {
            const Bitboard blockers = index_to_occupancy(magic.mask, j);
